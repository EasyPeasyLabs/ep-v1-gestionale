...
export const Fornitori: React.FC = () => {
    // FIX: Fetched 'sedi' to correctly calculate location counts for each supplier.
    const { fornitori, addFornitore, updateFornitore, deleteFornitore, sedi } = useMockData();
    ...
    const handleSaveFornitore = (fornitore: Fornitore) => {
        if (fornitore.id) {
            ...
            // FIX: Removed 'sedi' from destructuring as it's not a property of Fornitore.
            const { id, ...newFornitore } = fornitore;
            addFornitore(newFornitore as Omit<Fornitore, 'id'>);
        }
        handleCloseModal();
    };
                ...
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {fornitori.map(fornitore => {
                            // FIX: Calculated sede count by filtering the separate 'sedi' collection.
                            const sediCount = sedi.filter(s => s.fornitoreId === fornitore.id).length;
                            return (
                                <tr key={fornitore.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td className="px-6 py-4">{sediCount}</td>
