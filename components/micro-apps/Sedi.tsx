...
// FIX: Imported 'SedeAnagrafica' as 'Sede' to match the component's internal type usage.
import { SedeAnagrafica as Sede, Fornitore, Indirizzo } from '../../types';
// FIX: Imported 'EMPTY_SEDE_ANAGRAFICA' as 'EMPTY_SEDE' to match usage.
import { EMPTY_SEDE_ANAGRAFICA as EMPTY_SEDE } from '../../constants';
...
const SedeForm: React.FC<{
    ...
    fornitori: Fornitore[],
    // FIX: Updated onSave signature to pass the complete sede object.
    onSave: (sede: Sede | Omit<Sede, 'id'>) => void,
    onCancel: () => void
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // FIX: Pass only the form data object to onSave.
        onSave(formData);
    };
...
export const Sedi: React.FC = () => {
    // FIX: Fetched 'sedi' collection to work with the relational data model.
    const { fornitori, sedi, addSede, updateSede, deleteSede } = useMockData();
    ...
    // FIX: Simplified handler to accept a single 'sede' object, matching the new form logic.
    const handleSaveSede = (sede: Sede | Omit<Sede, 'id'>) => {
        if ('id' in sede && sede.id) {
            // FIX: Corrected call to updateSede with a single argument.
            updateSede(sede as Sede);
        } else {
            // FIX: Corrected call to addSede with a single argument.
            addSede(sede as Omit<Sede, 'id'>);
        }
        handleCloseModal();
    };
    
    // FIX: Simplified handler to accept only 'sedeId'.
    const handleDeleteSede = (sedeId: string) => {
        if(window.confirm('Sei sicuro di voler eliminare questa sede?')) {
            // FIX: Corrected call to deleteSede with a single argument.
            deleteSede(sedeId);
        }
    }
        ...
        <div className="p-4 md:p-8">
            ...
            <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-x-auto mt-4">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {/* FIX: Reworked render logic to iterate over 'sedi' and find the parent 'fornitore'. */}
                        {sedi.map(sede => {
                            const fornitore = fornitori.find(f => f.id === sede.fornitoreId);
                            return (
                                <tr key={sede.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" style={{borderLeft: `5px solid ${sede.colore || '#A0AEC0'}`}}>
                                    ...
                                    <td className="px-6 py-4">{fornitore ? fornitore.dati.ragioneSociale : 'N/A'}</td>
                                    <td className="px-6 py-4">{formatIndirizzo(sede.indirizzo)}</td>
                                    <td className="px-6 py-4">{sede.capienzaMassima}</td>
                                    <td className="px-6 py-4 text-right space-x-2">
                                        <button onClick={() => handleOpenModal(sede, sede.fornitoreId)} className="text-blue-600 hover:text-blue-800"><PencilIcon /></button>
                                        <button onClick={() => handleDeleteSede(sede.id)} className="text-red-600 hover:text-red-800"><TrashIcon /></button>
                                    </td>
                                </tr>
                            )
                        })}
                         {/* FIX: Correctly check if the 'sedi' array is empty. */}
                         {sedi.length === 0 && (
                             <tr>
