...
interface FinanceListViewProps {
    ...
    onEdit: (item: Invoice | Quote | Transaction) => void;
    onDelete: (id: string) => void;
    onPrint?: (item: Invoice | Quote | Transaction) => void;
    onSeal?: (item: Invoice) => void;
    onWhatsApp: (item: Invoice | Quote | Transaction) => void;
    onConvert?: (item: Invoice | Quote) => void;
...
const FinanceListView: React.FC<FinanceListViewProps> = ({ 
    ...
    const filteredList = useMemo(() => {
        let list: (Invoice | Quote | Transaction)[] = [];
        ...
        if (activeTab === 'transactions') {
            ...
            list = baseList.filter(i => {
                ...
                const matchesTab = activeTab === 'invoices' 
                    ? (isGhostFilter ? true : !(i as Invoice).isGhost) 
                    : ((i as Invoice).status === DocumentStatus.PendingSDI || (i as Invoice).status === DocumentStatus.SealedSDI || ((i as Invoice).sdiId && String((i as Invoice).sdiId).trim().length > 0));
                ...
                const d = new Date((i as Invoice).issueDate);
                const yearMatch = d.getFullYear() === filterYear;
                const monthMatch = filterMonth === 'all' || (d.getMonth() + 1) === parseInt(filterMonth);
            ...
            if (statusFilter && activeTab === 'invoices') {
                if (statusFilter === 'ghost') {
                    list = list.filter(i => (i as Invoice).isGhost === true);
                } else {
                    list = list.filter(i => (i as Invoice).status === statusFilter);
                }
            }
        }
    ...
    const getDate = (item: Invoice | Transaction | Quote) => {
        if ('date' in item) return new Date(item.date).getTime();
        if ('issueDate' in item) return new Date(item.issueDate).getTime();
        return 0;
    };

    const getNumber = (item: Invoice | Transaction | Quote) => {
...
const totals = useMemo(() => {
    if (activeTab === 'transactions') {
        return filteredList.reduce((acc, t) => {
            if ((t as Transaction).type === TransactionType.Income) acc.income += Number((t as Transaction).amount);
            else if ((t as Transaction).type === TransactionType.Expense) acc.expense += Number((t as Transaction).amount);
            return acc;
        }, { income: 0, expense: 0 });
    } else if (activeTab === 'invoices' || activeTab === 'archive') {
        // Solo fatture reali per calcolo fiscale
        const realInvoices = filteredList.filter(i => !(i as Invoice).isGhost);
        const gross = realInvoices.reduce((acc, i) => acc + (Number((i as Invoice).totalAmount) || 0), 0);
        const taxable = gross * 0.78;
        const stamps = realInvoices.reduce((acc, i) => acc + (Number((i as Invoice).totalAmount) > 77.47 ? 2 : 0), 0);
        return { gross, taxable, stamps };
    }
    return null;
}, [filteredList, activeTab]);

    const getDocumentNumber = (item: Invoice | Quote | Transaction) => {
        if ('invoiceNumber' in item) return (item as Invoice).invoiceNumber;
        if ('quoteNumber' in item) return (item as Quote).quoteNumber;
        if ('relatedDocumentId' in item && (item as Transaction).relatedDocumentId?.startsWith('FT-')) return (item as Transaction).relatedDocumentId;
        if ('transactionNumber' in item) return (item as Transaction).transactionNumber?.toString();
    ...
    const handleSendReportToSimona = () => {
        ...
        selectedInvoices.forEach(inv => {
            const dateStr = new Date('issueDate' in inv ? (inv as Invoice).issueDate : (inv as Transaction).date).toLocaleDateString('it-IT');
            message += `🔹 *Fattura:* ${'invoiceNumber' in inv ? (inv as Invoice).invoiceNumber : 'N/D'}\n   Data: ${dateStr}\n   SDI: ${'sdiId' in inv ? (inv as Invoice).sdiId : 'N/D'}\n\n`;
        });
    ...
    const handleBulkPrint = () => {
        if (!onPrint) return;
        let sourceList: (Invoice | Quote | Transaction)[] = [];
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredList.map(item => {
                            ...
                            const isIncome = isTransaction && ('type' in item ? (item as Transaction).type : undefined) === TransactionType.Income;
                            ...
                            <tr key={item.id} className={`hover:bg-indigo-50/30 transition-colors group ${selectedItems.includes(item.id) ? 'bg-indigo-50' : ''}`}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap font-medium text-slate-500">{new Date(('date' in item ? (item as Transaction).date : ('issueDate' in item ? (item as Invoice).issueDate : ''))).toLocaleDateString()}</td>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-1">
                                        ...
                                        {activeTab === 'quotes' && onActivate && (
                                            <button onClick={() => onActivate(item as Quote)} className="md-icon-btn text-indigo-600 bg-indigo-50 hover:bg-indigo-100" title="Attiva Progetto Ente"><BoltIcon /></button>
                                        )}
                                        {onConvert && (
