...
const LocationForm: React.FC<{ 
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSave({ 
            ...location, 
            name, address, city, capacity, rentalCost, distance, color, closedAt, isPubliclyVisible 
        });
    };
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mt-4 animate-fade-in">
            ...
            <div className="border-t border-gray-200 pt-3 mb-4">
                <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 flex items-center justify-between">
                    <div>
                        <h4 className="text-xs font-bold text-indigo-700 uppercase tracking-wider mb-1">Gestione Corsi e Slot</h4>
                        <p className="text-[10px] text-indigo-500">La gestione della disponibilità oraria è stata spostata nella pagina dedicata ai <strong>Corsi</strong>.</p>
                    </div>
                </div>
            </div>

            <div className="flex justify-end gap-2 mt-4">
                <button type="button" onClick={onCancel} className="md-btn md-btn-flat md-btn-sm text-gray-400">Annulla</button>
                <button type="button" onClick={handleSubmit} className="md-btn md-btn-raised md-btn-primary md-btn-sm font-bold uppercase tracking-widest text-[10px]">Salva Sede</button>
            </div>
        </div>
    );
};
...
const Suppliers: React.FC = () => {
    ...
    const fetchSuppliers = useCallback(async () => {
        try {
            setLoading(true);
            const [suppliersData, coursesData] = await Promise.all([
                getSuppliers(),
                getOpenCourses()
            ]);
    ...
    const getEarliestDay = (supplier: Supplier): number => {
        ...
        supplier.locations.forEach(loc => { 
            ...
            locCourses.forEach(course => { 
                const day = course.dayOfWeek === 0 ? 7 : course.dayOfWeek; 
        ...
        <div>
            ...
            {loading ? <div className="flex justify-center py-12"><Spinner /></div> : (
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{paginatedSuppliers.map(supplier => {
                ...
                return (
                    <div key={supplier.id} className={`md-card p-6 flex flex-col ${showTrash ? 'opacity-75' : ''} border-t-4 border-indigo-50`}>
                                        ...
                                        return (
                                            <li key={loc.id} className={`flex justify-between items-center p-1 rounded ${loc.closedAt ? 'bg-red-50 text-red-500 line-through' : 'bg-gray-50'}`}>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {loc.isPubliclyVisible === false && <EyeOffIcon className="w-2 h-2 text-gray-400" />}
                                                </div>
                                                <div className="flex gap-1 flex-wrap justify-end max-w-[50%]">
                                                    {locCourses.map((course, i) => (
                                                        <span key={i} className="px-1.5 py-0.5 border rounded text-[10px] font-bold flex items-center gap-0.5 bg-white text-indigo-600">
                                                            <span className={`text-[8px] px-0.5 rounded mr-0.5 ${
                                                                course.slotType === 'SG' ? 'bg-orange-100 text-orange-700' : 
                                                                course.slotType === 'EVT' ? 'bg-purple-100 text-purple-700' : 
                                                                'bg-blue-100 text-blue-700'
                                                            }`}>{course.slotType || 'LAB'}</span>
                                                            {daysMap[course.dayOfWeek].substring(0,3)}
                                                            <span className="text-[8px] text-gray-400 font-normal ml-0.5">({course.minAge || 0}-{course.maxAge || '?'}a)</span>
                                                        </span>
                                                    ))}
                                                </div>
                                            </li>
                        ...
                        <div className="mt-4 pt-4 border-t flex justify-between items-center space-x-2">
                            {!showTrash && (
                                <button onClick={() => setContractModalSupplier(supplier)} className="text-xs bg-indigo-50 text-indigo-700 px-2 py-1 rounded border border-indigo-200 hover:bg-indigo-100 flex items-center gap-1 font-bold">
